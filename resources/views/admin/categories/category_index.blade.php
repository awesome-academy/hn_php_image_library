...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">{{ ucfirst(__('categories')) }}</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ ucfirst(__('home')) }}</a>
                        </li>
                        <li class="breadcrumb-item active">{{ ucfirst(__('categories')) }}</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="d-flex flex-row mb-3">
                ...
                <div class="p-2 w-50">
                    <a role="button" class="btn btn-primary float-right"
                       href="{{ route('categories.create') }}">{{ ucfirst(__('create', ['name' => __('category')])) }}</a>
                </div>
            </div>
            @isset($categories)
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">
                            <h3 class="card-title">{{ ucfirst(__('categories')) }}</h3>
                        </div>
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    <a class="dropdown-item"
                                                       href="{{ route('categories.edit', [$value]) }}">{{ ucfirst(__('edit', ['name' => ''])) }}</a>
                                            ...
                                            <div class="modal fade" id="confirmModal{{ $value['id'] }}" tabindex="-1"
                                                ...
                                                <form class="modal-dialog"
                                                      action="{{ route('categories.destroy', [$value]) }}"
                                                      method="POST">
                                                    @csrf
                                                    @method('delete')
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <p> {{ __('confirm', ['action' => __('delete', ['name' => __('category')])]) }}
                                                            </p>
                                                        </div>
